...
public class BoardDAO {
	...
	public boolean createBoard(String tableName){
		...
		String sql = " CREATE TABLE BOARD_"+ tableName +" ( ";
			   sql+= " bo_idx int(10) auto_increment not null, ";
			   sql+= " bo_writer varchar(20), ";
			   sql+= " bo_member varchar(10), ";
			   sql+= " bo_subject varchar(1000), ";
			   sql+= " bo_content varchar(4000), ";
			   sql+= " bo_is_secret int(1), ";
			   sql+= " bo_is_notice int(1), ";
			   sql+= " bo_file varchar(20), ";
			   sql+= " bo_pass varchar(300), ";
			   sql+= " bo_ref int(10), ";
			   sql+= " bo_level int(10), ";
			   sql+= " bo_step int(5), ";
			   sql+= " bo_hit int(10), ";
			   sql+= " bo_regdate timestamp, ";
			   sql+= " bo_is_view int(1), ";
			   sql+= " bo_ip varchar(20), ";
			   sql+= " bo_1 varchar(200), ";
			   sql+= " bo_2 varchar(200), ";
			   sql+= " bo_3 varchar(200), ";
			   sql+= " bo_4 varchar(200), ";
			   sql+= " bo_5 varchar(200), ";
			   sql+= " bo_tag varchar(500), ";
			   sql+= " primary key(bo_idx) ";
			   sql+= " ) ";
		try{
			conn = conn = DBManager.getInstance().getConnection();
	...
	public void dropBoard(String tableName){
		String sql = "DROP TABLE BOARD_"+tableName;
		try{
			conn = conn = DBManager.getInstance().getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.execute();
	...
	public ArrayList<BoardVO> getBoardList(String tableName, int firstRow , int endRow, String sfl, String stx, String sst){
		...
		//페이징 처리
		String sql = " SELECT * FROM BOARD_" + tableName+"";
			   //검색이 있을 경우 
